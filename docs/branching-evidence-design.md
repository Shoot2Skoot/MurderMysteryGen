...
2.  **Invoke `NarrativeRefinementAgent` (3.1):**
    *   **Input:** `BranchingCaseContext` (initial state).
...
3.  **Invoke `(Optional) MapGeneratorAgent` (3.2) (if not pre-loaded):
    ...
    *   *Note: If using pre-defined maps, this step is skipped, and the map data is assumed to be loaded into `BranchingCaseContext.timeline.locations` beforehand or at initialization.*
